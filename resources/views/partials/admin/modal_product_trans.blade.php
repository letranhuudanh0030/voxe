        ...
        <div class="card">
            ...
            {{-- <form action=""> --}}
                <div class="card-body">
                    <div class="form-lang">
                        @if ($product)
                            @foreach ($product->language as $key => $item)
                            <div class="card language language-edit-{{ $key }}">
                                ...
                                <div class="card-body">
                                    <div class="form-group">
                                        <label for="name_lang">Tên sản phẩm:</label>
                                        <input type="text" id="name_lang" name="name_lang[]" class="form-control" placeholder="Nhập tên sản phẩm ngôn ngữ mới" value="{{ $item->pivot->title }}">
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <label for="content_lang">Nội dung:</label>
                                        <textarea id="content_lang" cols="30" rows="10" class="form-control config_content" name="content_lang[]" placeholder="Nhập quảng cáo ngôn ngữ mới">{{ $item->pivot->content }}</textarea>
                                    </div>
                                    <div class="form-group">
...
<script>
    ...
    $('.add-lang').click(function(){
        ...
        form += OpenFormGroup
        form += '<label for="name_lang'+ index +'">Tên sản phẩm:</label>'
        form += '<input type="text" id="name_lang'+ index +'" name="name_lang[]" class="form-control" placeholder="Nhập tên sản phẩm ngôn ngữ mới">'
        form += CloseDiv
        form += OpenFormGroup
        form += '<label for="short_desc_lang'+ index +'">Mô tả ngắn:</label>'
        form += '<textarea id="short_desc_lang'+ index +'" cols="30" rows="5" class="form-control config_content" name="short_desc_lang[]" placeholder="Nhập mô tả ngắn ngôn ngữ mới"></textarea>'
        form += CloseDiv
        form += OpenFormGroup
        form += '<label for="content_lang'+ index +'">Quảng cáo:</label>'
        form += '<textarea id="content_lang'+ index +'" cols="30" rows="5" class="form-control config_content" name="content_lang[]" placeholder="Nhập quảng cáo ngôn ngữ mới"></textarea>'
        form += CloseDiv
        form += OpenFormGroup
